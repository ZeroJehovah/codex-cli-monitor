...
- 只读打开 `~/.local/share/opencode/opencode.db`，把运行中的 `opencode` 进程绑定到会话行：
  ...
  `opencode -s <session-id>` 恢复参数，最后才回退到 `session.directory` 与进程 `cwd` 相同的会话行；
  如果同目录存在进程启动后创建的新行则优先选择它，否则允许绑定到已存在的（包括早于进程启动的）
  当前会话。`-s` 恢复锚点只是出发点而非绑定保证：若进程在此窗口内又创建了
  更新的会话（例如另起新对话），该新会话才是该进程当前真正所在的行，其开启回合会取代陈旧的已完成
  锚点，进程不会冻结在 `成功` 上；已被占用的行也被逐进程独占地认领。同一目录内的多个
  OpenCode 进程因此各自占用独立的行，而不会一起塌缩到某一个会话的状态上。多个没有 hook/`-s`
  锚点的同目录进程会先分配所有尚未占用的开放回合，再考虑已完成行；session 创建时间只在同为开放
  或同为完成的候选之间用于配对，避免较早创建但仍在运行的恢复会话被较新却已经完成的行挤掉。
...
进行中的会话；没有进行中会话时，选择最近结构化活动的行。因此，启动后继续工作的旧
会话仍能被识别，进程后来创建的新会话也不会被更早的失败行抢占，显式恢复的空闲会话
也能保留原来的结果。
