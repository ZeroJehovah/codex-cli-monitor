...
红色及相近颜色。色板中允许保留彼此接近的候选色，但按服务器名称和 ID 排序后，相邻
服务器不会使用视觉上接近的颜色；普通刷新期间颜色保持稳定，只有新增、移除服务器造
成新的相邻低对比组合时才会重选必要的颜色。该服务器的全部会话消失后会释放颜色，之
后重新出现时可以获得新的随机颜色。悬浮窗不再绘制单独的灰
